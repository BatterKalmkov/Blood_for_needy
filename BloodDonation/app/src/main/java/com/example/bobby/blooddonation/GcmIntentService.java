...
public class GcmIntentService extends IntentService {
    ...
    public static final String SUBSCRIBE2 = "subscribe2";
    public static final String UNSUBSCRIBE2 = "unsubscribe2";
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if(intent.getStringExtra(KEY2)!=null) {
            switch (key2) {
                case SUBSCRIBE2:
                    // subscribe to a topic
                    String topic = intent.getStringExtra(TOPIC2);
                    subscribeToTopic2(topic);
                    break;
                case UNSUBSCRIBE2:
                    break;
                default:
    ...
    private void sendRegistrationToServer2(final String token) {
        ...
        // checking for valid login session
        User2 user = MyApplication.getInstance2().getPrefManager2().getUser2();
        if (user == null) {
            // TODO
            // user not found, redirecting him to login screen
            return;
        ...
        String endPoint = EndPoints.USER2.replace("_ID_", user.getId());
     ...
     * Subscribe to a topic
     */
    public static void subscribeToTopic2(String topic) {
        GcmPubSub pubSub = GcmPubSub.getInstance(MyApplication.getInstance().getApplicationContext());
        InstanceID instanceID = InstanceID.getInstance(MyApplication.getInstance2().getApplicationContext());
        String token = null;
        try {
            token = instanceID.getToken(MyApplication.getInstance2().getApplicationContext().getString(R.string.gcm_defaultSenderId),
                    GoogleCloudMessaging.INSTANCE_ID_SCOPE, null);
            if (token != null) {
                pubSub.subscribe(token, "/topics/" + topic, null);
                Log.e(TAG, "Subscribed to topic: " + topic);
            ...
            Log.e(TAG, "Topic subscribe error. Topic: " + topic + ", error: " + e.getMessage());
            Toast.makeText(MyApplication.getInstance2().getApplicationContext(), "Topic subscribe error. Topic: " + topic + ", error: " + e.getMessage(), Toast.LENGTH_SHORT).show();
        }
    }

    public void unsubscribeFromTopic2(String topic) {
        GcmPubSub pubSub = GcmPubSub.getInstance(getApplicationContext());
        InstanceID instanceID = InstanceID.getInstance(getApplicationContext());
        String token = null;
        try {
